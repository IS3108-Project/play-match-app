...
export default function ActivityCard({
  activity,
  isHost = false, //TODO: update isHost logic
}: ActivityCardProps) {
  return (
    <div className="relative flex flex-row overflow-hidden rounded-lg border bg-card shadow-sm lg:flex-col">
      ...
      <div className="basis-[45%] shrink-0 grow-0 bg-muted lg:basis-auto lg:w-full">
        <img
          src={
            activity.imageSrc || "/assets/default-activity-image/default.png"
          }
      ...
      <div className="basis-[55%] min-w-0 p-4 lg:basis-auto">
        ...
        {activity.status === "joined" && (
          <ActivityDetailsCard
            activity={activity}
            isHost={isHost} //TODO: update isHost logic
            status="joined"
          >
            <Button className="mt-3 w-full" size="sm" variant="outline">
              {isHost ? "Manage Activity" : "View More"}
            </Button>
          </ActivityDetailsCard>
        )}
